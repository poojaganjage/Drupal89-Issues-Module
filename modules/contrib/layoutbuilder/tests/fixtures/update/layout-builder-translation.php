...
$display['third_party_settings']['layoutbuilder']['sections'][] = $section_array_default;
...
// @see \Drupal\Tests\layoutbuilder\Functional\Update\Translatability\MakeLayoutUntranslatableUpdatePathTestBase
foreach ($this->layoutBuilderTestCases as $bundle => $test_case) {
  if ($test_case['has_layout']) {
    $values_en = [
      ...
      'langcode' => 'en',
      'delta' => '0',
      'layoutbuilder__layout_section' => serialize(Section::fromArray($section_array_default)),
    ];

    // Add the layout data to the node.
    $connection->insert('node__layoutbuilder__layout')
      ->fields(array_keys($values_en))
      ->values($values_en)
      ->execute();
    $connection->insert('node_revision__layoutbuilder__layout')
      ->fields(array_keys($values_en))
      ->values($values_en)
      ->execute();
  }
