...
class DefaultsSectionStorageTest extends KernelTestBase {
  ...
  protected function setUp() {
    ...
    $this->installConfig(['layoutbuilder_defaults_test']);
  ...
  public function testConfigInstall() {
    /** @var \Drupal\layoutbuilder\Entity\LayoutEntityDisplayInterface $display */
  ...
  public function testAccess($expected, $operation, $is_enabled, array $section_data) {
    ...
    $display
      ->setThirdPartySetting('layoutbuilder', 'sections', $section_data)
      ->save();
  ...
  public function providerTestAccess() {
    $section_data = [
      new Section(
        ...
        ['layoutbuilder_defaults_test' => ['which_party' => 'third']]
      ),
    ];
  ...
  public function testGetContextsDuringPreview() {
    ...
    $this->assertSame(['view_mode', 'display', 'layoutbuilder.entity'], array_keys($result));

    $this->assertSame($context, $result['display']);

    $this->assertInstanceOf(EntityContext::class, $result['layoutbuilder.entity']);
    $result_value = $result['layoutbuilder.entity']->getContextValue();
   ...
   *
   * @expectedDeprecation \Drupal\layoutbuilder\SectionStorageInterface::setSectionList() is deprecated in Drupal 8.7.0 and will be removed before Drupal 9.0.0. This method should no longer be used. The section list should be derived from context. See https://www.drupal.org/node/3016262.
   * @group legacy
   */
  public function testSetSectionList() {
    $section_list = $this->prophesize(SectionListInterface::class);
    $this->expectException(\Exception::class);
    $this->expectExceptionMessage('\Drupal\layoutbuilder\SectionStorageInterface::setSectionList() must no longer be called. The section list should be derived from context. See https://www.drupal.org/node/3016262.');
  ...
  public function testLoadFromDisplay() {
    ...
    $section_storage_manager = $this->container->get('plugin.manager.layoutbuilder.section_storage');
