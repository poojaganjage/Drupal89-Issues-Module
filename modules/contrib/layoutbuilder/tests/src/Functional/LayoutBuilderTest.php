...
class LayoutBuilderTest extends BrowserTestBase {
  ...
  public function testOverrides() {
    ...
    $components = Node::load(1)->get('layoutbuilder__layout')->getSection(0)->getComponents();
    end($components);
    $uuid = key($components);

    $this->drupalGet('layoutbuilder/update/block/overrides/node.1/0/content/' . $uuid);
   ...
   *
   * @see \Drupal\layoutbuilder\Plugin\SectionStorage\OverridesSectionStorage::getDefaultSectionStorage()
   */
  ...
  public function testLayoutBuilderChooseBlocksAlter() {
    // See layoutbuilder_test_plugin_filter_block__layoutbuilder_alter().
    $assert_session = $this->assertSession();
  ...
  public function testCustomSectionAttributes() {
    ...
    $page->pressButton('Add section');
    // See \Drupal\layoutbuilder_test\Plugin\Layout\LayoutBuilderTestPlugin::build().
   ...
   * @see \Drupal\layoutbuilder\EventSubscriber\BlockComponentRenderArray::onBuildRender()
   */
   ...
   *
   * @see \Drupal\layoutbuilder_test\Plugin\SectionStorage\SimpleConfigSectionStorage
   */
  public function testSimpleConfigBasedLayout() {
    ...
    $this->container->get('config.factory')->getEditable('layoutbuilder_test.test_simple_config.existing')
  ...
  protected function assertCorrectLayouts() {
    $assert_session = $this->assertSession();
    // Ensure the layouts provided by layoutbuilder are available.
    $expected_layouts_hrefs = [
      'layoutbuilder/configure/section/overrides/node.1/0/layout_onecol',
      'layoutbuilder/configure/section/overrides/node.1/0/layout_twocol_section',
      'layoutbuilder/configure/section/overrides/node.1/0/layout_threecol_section',
      'layoutbuilder/configure/section/overrides/node.1/0/layout_fourcol_section',
    ];
    ...
    foreach ($unexpected_layouts as $unexpected_layout) {
      $assert_session->linkByHrefNotExists("layoutbuilder/add/section/overrides/node.1/0/$unexpected_layout");
      $assert_session->linkByHrefNotExists("layoutbuilder/configure/section/overrides/node.1/0/$unexpected_layout");
    }
  }
