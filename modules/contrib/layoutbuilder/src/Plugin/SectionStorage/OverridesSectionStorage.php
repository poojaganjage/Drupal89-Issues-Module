 ...
 *   \Drupal\layoutbuilder\Plugin\SectionStorage\DefaultsSectionStorage.
 ...
 *       "EntityHasField" = \Drupal\layoutbuilder\Plugin\SectionStorage\OverridesSectionStorage::FIELD_NAME,
 *     }),
...
class OverridesSectionStorage extends SectionStorageBase implements ContainerFactoryPluginInterface, OverridesSectionStorageInterface, SectionStorageLocalTaskProviderInterface {
   ...
   * @var string
   */
  const FIELD_NAME = 'layoutbuilder__layout';
  ...
  public static function create(ContainerInterface $container, array $configuration, $plugin_id, $plugin_definition) {
    return new static(
      ...
      $container->get('plugin.manager.layoutbuilder.section_storage'),
  ...
  public function extractIdFromRoute($value, $definition, $name, array $defaults) {
    @trigger_error('\Drupal\layoutbuilder\SectionStorageInterface::extractIdFromRoute() is deprecated in Drupal 8.7.0 and will be removed before Drupal 9.0.0. \Drupal\layoutbuilder\SectionStorageInterface::deriveContextsFromRoute() should be used instead. See https://www.drupal.org/node/3016262.', E_USER_DEPRECATED);
  ...
  public function getSectionListFromId($id) {
    @trigger_error('\Drupal\layoutbuilder\SectionStorageInterface::getSectionListFromId() is deprecated in Drupal 8.7.0 and will be removed before Drupal 9.0.0. The section list should be derived from context. See https://www.drupal.org/node/3016262.', E_USER_DEPRECATED);
    if (strpos($id, '.') !== FALSE) {
   ...
   *
   * @see \Drupal\layoutbuilder\SectionStorageInterface::deriveContextsFromRoute()
  ...
  public function buildLocalTasks($base_plugin_definition) {
    $local_tasks = [];
    foreach ($this->getEntityTypes() as $entity_type_id => $entity_type) {
      $local_tasks["layoutbuilder.overrides.$entity_type_id.view"] = $base_plugin_definition + [
        'route_name' => "layoutbuilder.overrides.$entity_type_id.view",
        'weight' => 15,
        'title' => $this->t('Layout'),
        'base_route' => "entity.$entity_type_id.canonical",
        'cache_contexts' => ['layoutbuilder_is_active:' . $entity_type_id],
      ];
    }
    return $local_tasks;
  }
  ...
  protected function getEntityTypes() {
    return array_filter($this->entityTypeManager->getDefinitions(), function (EntityTypeInterface $entity_type) {
      return $entity_type->entityClassImplements(FieldableEntityInterface::class) && $entity_type->hasHandlerClass('form', 'layoutbuilder') && $entity_type->hasViewBuilderClass() && $entity_type->hasLinkTemplate('canonical');
    });
  }
  ...
  public function getLayoutBuilderUrl($rel = 'view') {
    ...
    return Url::fromRoute("layoutbuilder.{$this->getStorageType()}.{$this->getEntity()->getEntityTypeId()}.$rel", $route_parameters);
  }
  ...
  public function getContextsDuringPreview() {
    ...
    if (isset($contexts['entity'])) {
      $contexts['layoutbuilder.entity'] = $contexts['entity'];
