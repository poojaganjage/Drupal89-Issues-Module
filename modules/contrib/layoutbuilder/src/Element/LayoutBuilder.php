...
namespace Drupal\layoutbuilder\Element;
...
use Drupal\Core\Url;
use Drupal\layoutbuilder\Context\LayoutBuilderContextTrait;
...
class LayoutBuilder extends RenderElement implements ContainerFactoryPluginInterface {
  ...
  public function preRender($element) {
    if ($element['#section_storage'] instanceof SectionStorageInterface) {
      $element['layoutbuilder'] = $this->layout($element['#section_storage']);
    }
    return $element;
  }
  ...
  protected function layout(SectionStorageInterface $section_storage) {
    ...
    $output[] = $this->buildAddSectionLink($section_storage, $count);
    $output['#attached']['library'][] = 'layoutbuilder/drupal.layoutbuilder';
  ...
  protected function buildAddSectionLink(SectionStorageInterface $section_storage, $delta) {
    ...
    return [
      'link' => [
        '#type' => 'link',
        '#title' => $title,
        '#url' => Url::fromRoute('layoutbuilder.choose_section',
          [
  ...
  protected function buildAdministrativeSection(SectionStorageInterface $section_storage, $delta) {
    ...
    foreach ($layout_definition->getRegions() as $region => $info) {
        ...
        foreach (Element::children($build[$region]) as $uuid) {
          ...
          $build[$region][$uuid]['#contextual_links'] = [
            'layoutbuilder_block' => [
              ...
              // @see layoutbuilder.links.contextual.yml
              'metadata' => [
      ...
      $build[$region]['layoutbuilder_add_block']['link'] = [
        '#type' => 'link',
        ...
        '#url' => Url::fromRoute('layoutbuilder.choose_block',
          [
            'section_storage_type' => $storage_type,
            'section_storage' => $storage_id,
            'delta' => $delta,
      ...
      $build[$region]['layoutbuilder_add_block']['#type'] = 'container';
      $build[$region]['layoutbuilder_add_block']['#attributes'] = [
        'class' => ['layout-builder__add-block'],
        'data-layout-builder-highlight-id' => $this->blockAddHighlightId($delta, $region),
      ];
      $build[$region]['layoutbuilder_add_block']['#weight'] = 1000;
    ...
    $build['#attributes']['data-layout-update-url'] = Url::fromRoute('layoutbuilder.move_block', [
      'section_storage_type' => $storage_type,
      'section_storage' => $storage_id,
    ])->toString();
    ...
    return [
      ...
      'remove' => [
        ...
        '#url' => Url::fromRoute('layoutbuilder.remove_section', [
          'section_storage_type' => $storage_type,
          'section_storage' => $storage_id,
          'delta' => $delta,
        ]),
      ...
      'configure' => [
        ...
        '#access' => $layout instanceof PluginFormInterface,
        '#url' => Url::fromRoute('layoutbuilder.configure_section', [
          'section_storage_type' => $storage_type,
          'section_storage' => $storage_id,
          'delta' => $delta,
        ]),
