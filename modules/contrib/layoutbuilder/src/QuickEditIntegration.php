...
class QuickEditIntegration implements ContainerInjectionInterface {
   ...
   * @see layoutbuilder_quickedit_render_field()
   */
  public function entityViewAlter(array &$build, EntityInterface $entity, EntityViewDisplayInterface $display) {
    if (!$entity instanceof FieldableEntityInterface || !isset($build['_layoutbuilder'])) {
      return;
    }
    ...
    foreach (Element::children($build['_layoutbuilder']) as $delta) {
      $section = $build['_layoutbuilder'][$delta];
    ...
    foreach ($plugin_ids_to_update as $delta => $regions) {
      foreach ($regions as $region => $uuids) {
        foreach ($uuids as $uuid => $component) {
          $build['_layoutbuilder'][$delta][$region][$uuid]['content']['#view_mode'] = static::getViewModeId($entity, $display, $delta, $uuid, $sections_hash);
        }
      }
    }
    ...
    foreach (Element::children($build) as $field_name) {
      if ($field_name !== '_layoutbuilder') {
        $field_build = &$build[$field_name];
        if (isset($field_build['#view_mode'])) {
          $field_build['#view_mode'] = "layoutbuilder-{$display->getMode()}-non_component-$sections_hash";
        }
      }
    }
  }
   ...
   *
   * @see \Drupal\layoutbuilder\QuickEditIntegration::deconstructViewModeId()
   */
   ...
   *
   * @see \Drupal\layoutbuilder\QuickEditIntegration::getViewModeId()
   */
  ...
  public function quickEditRenderField(FieldableEntityInterface $entity, $field_name, $quick_edit_view_mode_id, $langcode) {
    ...
    if (isset($entity_build['_layoutbuilder'][$delta])) {
      foreach (Element::children($entity_build['_layoutbuilder'][$delta]) as $region) {
        if (isset($entity_build['_layoutbuilder'][$delta][$region][$component_uuid])) {
          return $entity_build['_layoutbuilder'][$delta][$region][$component_uuid]['content'];
        }
      }
    }

    $this->getLogger('layoutbuilder')->warning('The field "%field" failed to render.', ['%field' => $field_name]);
    return [];
  }
   ...
   *
   * @see \Drupal\layoutbuilder\Plugin\Block\FieldBlock
   */
