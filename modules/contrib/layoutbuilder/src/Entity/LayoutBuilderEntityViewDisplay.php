...
use Drupal\layoutbuilder\LayoutEntityHelperTrait;
use Drupal\layoutbuilder\Plugin\SectionStorage\OverridesSectionStorage;
use Drupal\layoutbuilder\QuickEditIntegration;
...
class LayoutBuilderEntityViewDisplay extends BaseEntityViewDisplay implements LayoutEntityDisplayInterface {
  ...
  public function isOverridable() {
    return $this->isLayoutBuilderEnabled() && $this->getThirdPartySetting('layoutbuilder', 'allow_custom', FALSE);
  }
  ...
  public function setOverridable($overridable = TRUE) {
    $this->setThirdPartySetting('layoutbuilder', 'allow_custom', $overridable);
  ...
  public function isLayoutBuilderEnabled() {
    ...
    return (bool) $this->getThirdPartySetting('layoutbuilder', 'enabled');
  }
  ...
  public function enableLayoutBuilder() {
    $this->setThirdPartySetting('layoutbuilder', 'enabled', TRUE);
    return $this;
  }
  ...
  public function disableLayoutBuilder() {
    ...
    $this->setThirdPartySetting('layoutbuilder', 'enabled', FALSE);
    return $this;
  }
  ...
  public function getSections() {
    return $this->getThirdPartySetting('layoutbuilder', 'sections', []);
  }
  ...
  protected function setSections(array $sections) {
    ...
    // empty array.
    if (!$sections) {
      $this->unsetThirdPartySetting('layoutbuilder', 'sections');
    }
    else {
      $this->setThirdPartySetting('layoutbuilder', 'sections', array_values($sections));
    }
    return $this;
  }
  ...
  protected function removeSectionField($entity_type_id, $bundle, $field_name) {
    $query = $this->entityTypeManager()->getStorage($this->getEntityTypeId())->getQuery()
      ...
      ->condition('third_party_settings.layoutbuilder.allow_custom', TRUE);
  ...
  public function buildMultiple(array $entities) {
    ...
    foreach ($entities as $id => $entity) {
      $build_list[$id]['_layoutbuilder'] = $this->buildSections($entity);
      ...
      if (!Element::isEmpty($build_list[$id]['_layoutbuilder'])) {
  ...
  protected function buildSections(FieldableEntityInterface $entity) {
    ...
    $contexts['layoutbuilder.entity'] = EntityContext::fromEntity($entity, $label);
   ...
   *   The entity.
   *
   * @return \Drupal\layoutbuilder\Section[]
   *   The sections.
   *
   * @deprecated in drupal:8.7.0 and is removed from drupal:9.0.0.
   *   \Drupal\layoutbuilder\SectionStorage\SectionStorageManagerInterface::findByContext()
  ...
  protected function getRuntimeSections(FieldableEntityInterface $entity) {
    @trigger_error('\Drupal\layoutbuilder\Entity\LayoutBuilderEntityViewDisplay::getRuntimeSections() is deprecated in Drupal 8.7.0 and will be removed before Drupal 9.0.0. \Drupal\layoutbuilder\SectionStorage\SectionStorageManagerInterface::findByContext() should be used instead. See https://www.drupal.org/node/3022574.', E_USER_DEPRECATED);
  ...
  public function setComponent($name, array $options = []) {
    ...
    if ($is_view_configurable_non_extra_field || isset($extra_fields['display'][$name])) {
      $configuration = [
        'label_display' => '0',
        'context_mapping' => ['entity' => 'layoutbuilder.entity'],
      ];
   ...
   *
   * @return \Drupal\layoutbuilder\SectionStorage\SectionStorageManagerInterface
   *   The section storage manager.
   */
  private function sectionStorageManager() {
    return \Drupal::service('plugin.manager.layoutbuilder.section_storage');
  }
   ...
   *
   * @return \Drupal\layoutbuilder\SectionComponent|null
   *   The section component if it is available.
   *
   * @see \Drupal\layoutbuilder\QuickEditIntegration::entityViewAlter()
  ...
  private function getQuickEditSectionComponent() {
    ...
    if ($original_mode = $this->getOriginalMode()) {
      ...
      // \Drupal\layoutbuilder\QuickEditIntegration::entityViewAlter()
      ...
      if (count($parts) === 6 && $parts[0] === 'layoutbuilder') {
