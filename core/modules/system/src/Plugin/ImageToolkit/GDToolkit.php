...
class GDToolkit extends ImageToolkitBase {
  ...
  protected function loadSvg() {
    // Change public file uri to realpath.
    ...
    $this->svg = NULL;
    if (substr($mime_type, 0, strlen('image/svg')) === 'image/svg') {
      ...
      if ($file_contents && $data = simplexml_load_string($file_contents)) {
        ...
        foreach ($data->attributes() as $attribute => $value) {
          if (in_array($attribute, $needed_variables)) {
            $this->svg[$attribute] = (integer) $value;
          }
        }
  ...
  public function isValid() {
    return ((bool) $this->preLoadInfo || (bool) $this->resource || (bool) $this->getSvg());
  }
  ...
  public function getWidth() {
    ...
    elseif ($svg_data = $this->getSvg()) {
      return $svg_data['width'] ?? NULL;
    }
    else {
      return NULL;
    }
